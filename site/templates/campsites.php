        ...
        <?php slot() ?>
            <?php foreach ($page->children()->listed()->shuffle() as $campsite) : ?>
                <div class="bg-gray-100 border border-gray-200 rounded-lg shadow">
                    ...
                    <div class="p-5">
                        <a href="<?= $campsite->link() ?>" target="_blank">
                            <h3 class="mb-2 text-2xl font-bold tracking-tight text-sky-900"> <?= $campsite->title() ?></h3>
                        </a>

                        <hr class="h-px my-2 bg-gray-200 border-0">
                        
                        <p class="mb-1 text-base font-normal text-black">
                            <span class="text-sky-700"><?= $campsite->beach() ?></span> to the beach.<br>
                            <span class="text-sky-700"><?= $campsite->centre() ?></span> into Conil old town.<br>
                        </p>
                        <p class="mb-2 text-base font-normal text-gray-500">

                            <?php if ($campsite->tents()->isNotEmpty()) : ?>
                                Pitches For Tents: <span class="text-sky-700"><?= $campsite->tents() ?></span><br>
                            <?php endif ?>

                            <?php if ($campsite->bungalows()->isNotEmpty()) : ?>
                                Bungalows: <span class="text-sky-700"><?= $campsite->bungalows() ?></span><br>
                            <?php endif ?>

                            <?php if ($campsite->campervans()->isNotEmpty()) : ?>
                                Pitches for Campervans: <span class="text-sky-700"><?= $campsite->campervans() ?></span><br>
                            <?php endif ?>

                            <?php if ($campsite->pool()->isNotEmpty()) : ?>
                                Pool: <span class="text-sky-700"><?= $campsite->pool() ?></span><br>
                            <?php endif ?>

                            <?php if ($campsite->restaurant()->isNotEmpty()) : ?>
                                Restaurant: <span class="text-sky-700"><?= $campsite->restaurant() ?></span><br>
                            <?php endif ?>

                            <?php if ($campsite->shop()->isNotEmpty()) : ?>
                                Shop: <span class="text-sky-700"><?= $campsite->shop() ?></span><br>
                            <?php endif ?>
